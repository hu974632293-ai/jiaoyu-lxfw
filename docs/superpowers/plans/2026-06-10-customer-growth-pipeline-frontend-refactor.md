...
8. 官网首页必须优先表达企业可信度、服务对象、核心业务、项目活动和咨询转化，不得做成后台功能清单。
9. 官网 CTA 必须区分咨询/报名/查看项目与登录后台。
10. 官网不得展示 CRM 客户列表、客户 360、员工日报、学生心理预警明细、审计日志、权限矩阵、OpenAPI、seed、接口健康等内部信息。
11. 登录后后台才展示增长总览、客户增长、客户 360、二期助手和系统治理。
12. 当前阶段登录可用演示角色跳转，但文案必须明确真实认证、Token 和后端权限校验属于 V2 增强。
...
- 官网参考原则：同类教育服务官网只作为信息组织参考；本项目不复制参考站视觉或文案，重点吸收“信任建立、服务解释、项目/活动承接、咨询转化、后台登录分离”的结构。
...
**产品要求：** 官网不是后台能力展示页。第一屏必须让陌生游客快速理解企业定位、服务对象和下一步动作；CRM、客户 360、系统治理和演示控制只在登录后出现。
...
- 顶部公开导航。
- 首页首屏：企业定位、服务对象、核心承诺、咨询 CTA、登录后台入口。
- 企业介绍。
- 业务服务。
- 信任背书：服务流程、项目资源、顾问/老师能力或可追踪服务体系。
- 项目/课程展示。
- 活动/讲座。
- 知识/FAQ。
- 联系我们。
...
10. 官网不展示内部运营数据。
11. 官网首页不是后台功能陈列页，能清楚完成“建立信任 -> 解释服务 -> 引导咨询/项目/活动/登录”的路径。
12. `npm.cmd run build` 通过。
13. 最终阶段 `python -m pytest -v` 通过，确认未破坏已有后端 API。
...
| 官网变成普通营销空壳 | 首页必须包含服务对象、业务服务、项目/活动承接、FAQ 和咨询入口 |
| 官网又变成功能陈列柜 | 后台能力只转译为服务结果，CRM、客户 360、OpenAPI 和系统治理只在登录后展示 |
