...
2. 是否允许当前阶段只使用 `Base.metadata.create_all()` 建表。
   - 当前推荐：不作为生产建表路径；仅保留本地开发、SQLite 兼容和测试初始化用途。
...
| migration 管理 | 已引入 Alembic baseline，当前 head 为 `19582c185ae7` | 生产化建表使用 `python -m alembic upgrade head`；既有库确认 schema 后可 `stamp head` |
