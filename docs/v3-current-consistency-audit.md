...
11. 后续角色 Agent 体验应收敛为统一自然语言对话入口，场景 chip、快捷入口和能力卡只能作为提示和加速，不应成为用户必须先理解的动作标签；后端仍必须保留内部 action 类型、角色能力清单、确认契约、权限校验、幂等和审计。
12. Agent MVP 验收应优先选一条真实业务对象主链路，证明“识别意图 -> 读取真实上下文 -> 生成可确认动作 -> 用户确认 -> 调用正式 service 写入 -> 工作台刷新后可追踪”，不能再用“能发消息/能返回文案”作为完成标准。
