...
from app.core.auth import get_current_token_user, get_current_user
...
def require_token_permission(permission_code: str) -> Callable[..., SysUser]:
    def dependency(
        current_user: SysUser = Depends(get_current_token_user),
        db: Session = Depends(get_db),
    ) -> SysUser:
        ensure_default_admin_data(db)
        if not user_has_permission(db, current_user.username, permission_code):
